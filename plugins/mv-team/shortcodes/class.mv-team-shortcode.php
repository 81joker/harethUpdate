...
if (! class_exists('MV_Team_Shortcode')) {
    class MV_Team_Shortcode{

        public function __construct()
        {
            // add_shortcode( $tag:string, $callback:callable )
            add_shortcode( 'mv_team', array($this,  'add_shortcode' ));
        }
        ...
        public function add_shortcode($atts = array() , $content = null , $tag = ''){
            ...
            ob_start();
            require( MV_Team_PATH . 'views/mv-team_shortcode.php' );
            wp_enqueue_script( 'mv-team-main-jq' );
            // wp_enqueue_script( 'mv-team-options-js' );
            wp_enqueue_style( 'mv-team-main-css' );
            wp_enqueue_style( 'mv-team-style-css' );
            // mv_slider_options();
            return ob_get_clean();
        }
